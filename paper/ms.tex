...
period, could be a powerful means for measuring the ages of low-mass stars in
    the Milky Way.
However, although significant progress has recently been made in the
    calibration of empirical and semi-empirical gyrochronology models, the
    rotational evolution of K and M dwarfs is still poorly understood.
This is because the ages and rotation periods of low-mass dwarfs are often
...
In this paper we use the Gaia positions, parallaxes and proper motions to
    infer the three-dimensional velocities of stars in the Kepler field.
Based on their vertical velocities, we then calculate the kinematic ages of
    Kepler stars with measured rotation periods.
Finally, we use the kinematic ages of these Kepler field stars, plus stars in
    open clusters, to calibrate a new fully empirical gyrochronology relation
    that captures the complex rotational evolution of cool dwarfs over a range
    of masses and ages.
This gyrochronology model uses a Gaussian process to augment a power-law
    relation, in order to capture the time and color-dependence of rotational
    evolution.
Using cross validation, we demonstrate that this relation predicts ages for
    the GKM dwarfs in our sample to within XX\%.
This new gyrochronology relation predicts the age of stars from their
    (dereddened) Gaia colors, rotation periods, and rotation period
    uncertainties.
It applies to many but not all GKM dwarfs and we provide a full description of
scenarios in which this relation should and should not be used.
\end{abstract}
